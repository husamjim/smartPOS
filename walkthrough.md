...
### 7. التوطين العالمي وإزالة فرضيات السوق المحلي (Global Localization & Decoupling)
* **قاعدة بيانات التوطين (`countries.ts`)**:
  * بناء مرجع متكامل لجميع دول العالم مبني على معايير ISO-3166 يتضمن العملات الرسمية، الرموز الهاتفية، المناطق الزمنية، والمدن الكبرى.
* **إعدادات المتجر المتطورة (`Settings.tsx` & `SetupWizard.tsx`)**:
  * تفعيل اختيار ديناميكي للدولة يقوم تلقائياً بتهيئة العملة ومفتاح الاتصال الدولي وصيغ الأرقام المقابلة، وتصفية المدن بناء على الدولة المختارة.
  * تمكين التعديل الكامل لاسم الضريبة ونسبتها (مثل VAT, GST, Sales Tax) محلياً بدلاً من فرض القيمة والاسم في السوق السعودي.
  * إتاحة تخصيص صيغ الأرقام والعملات (Locale) وصيغ التاريخ ونظام التقويم المعتمد (ميلادي أو هجري).
* **عولمة واجهات البيع والمحاسبة والتقارير (`POS.tsx`, `Reports.tsx`, `CRM.tsx`, `ERP.tsx`)**:
  * استبدال رمز العملة الثابت (SAR) ومسميات الفروع والجمارك بنصوص ديناميكية تعتمد كلياً على عملة وهوية المنشأة النشطة في قاعدة البيانات.
  * ربط الرسوم البيانية لتقرير أداء الفروع في لوحة المؤشرات ببيانات الفروع الفعلية المخزنة بديناميكية تامة.
  * تحديث وتطهير القوالب الترحيبية ونماذج الاستيراد والبيانات التجريبية لتصبح ملائمة للتشغيل الدولي الفوري.
