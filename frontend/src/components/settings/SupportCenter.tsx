  ...
  return (
    <div className="space-y-6 text-right font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-slate-800 pb-3 overflow-x-auto">
        {[
          { id: 'faq', label: isRtl ? 'الأسئلة الشائعة' : 'FAQ Accordion', icon: HelpCircle },
          { id: 'guide', label: isRtl ? 'دليل الاستخدام' : 'User Manual', icon: BookOpen },
          { id: 'videos', label: isRtl ? 'فيديوهات تعليمية' : 'Video Tutorials', icon: Video },
          { id: 'contact', label: isRtl ? 'معلومات الدعم' : 'Contact Support', icon: PhoneCall },
          { id: 'ticket', label: isRtl ? 'إرسال تذكرة / اقتراح' : 'Submit Ticket', icon: MessageSquare },
        ].map(tab => {
          const Icon = tab.icon;
          return (
            <button
      ...
      {activeTab === 'guide' && (
        <div className="glass-card p-5 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 space-y-4 text-xs">
          <div className="space-y-3 leading-relaxed">
            <h4 className="font-bold text-sm text-slate-850 dark:text-slate-200 border-b pb-2">{isRtl ? '📘 دليل البدء السريع وإعداد الكاشير' : '📘 Quick Start POS Documentation'}</h4>
            
            <div className="space-y-3 text-slate-500">
              <p><strong>1. {isRtl ? 'تعريف المنتجات:' : 'Item Setup:'}</strong> {isRtl ? 'اذهب لـ ERP ← المنتجات، ثم أضف الباركود والسعر والتكلفة وحد الطلب الأدنى.' : 'Add items via ERP Products tab specifying the barcode, price, unit type, and reorder levels.'}</p>
              <p><strong>2. {isRtl ? 'بدء وردية البيع:' : 'Open POS Session:'}</strong> {isRtl ? 'في شاشة الكاشير POS، أدخل رصيد الصندوق الافتتاحي لبدء استقبال فواتير البيع.' : 'Start the POS cashier layout and input the initial float cash drawer value.'}</p>
              <p><strong>3. {isRtl ? 'إكمال الفاتورة:' : 'Completing Sale:'}</strong> {isRtl ? 'امسح الباركود، اختر طريقة الدفع (كاش، بطاقة، دفع مقسم)، ثم اطبع الفاتورة الحرارية.' : 'Scan barcode, select payment method (cash, card, split billing), and submit the receipt.'}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'videos' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            { title: isRtl ? 'تهيئة المتجر والمستودعات' : 'Store Setup Guide', duration: '3:45' },
            { title: isRtl ? 'شرح نقطة البيع للمطاعم والتجزئة' : 'POS Checkout Walkthrough', duration: '5:20' }
          ].map((vid, idx) => (
            <div key={idx} className="glass-card p-4 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 space-y-3 flex flex-col justify-between">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">{isRtl ? 'فيديو تعليمي تفصيلي خطوة بخطوة.' : 'Detailed video tutorial walk-through.'}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab === 'contact' && (
        <div className="glass-card p-5 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 space-y-4 text-xs font-semibold">
          <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200 border-b pb-2">{isRtl ? '📞 قنوات تواصل الدعم الفني للمؤسسات' : '📞 Enterprise support channels'}</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/15 space-y-1">
              <span className="text-2xl">☎️</span>
              <p className="text-slate-400 block text-[10px]">{isRtl ? 'هاتف الدعم المباشر' : 'Direct Phone'}</p>
              <p className="text-slate-800 dark:text-slate-250 font-mono">{supportPhone}</p>
            </div>
            <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/15 space-y-1">
              <span className="text-2xl">✉️</span>
              <p className="text-slate-400 block text-[10px]">{isRtl ? 'البريد الإلكتروني' : 'Email Address'}</p>
              <p className="text-slate-800 dark:text-slate-250 font-mono">{supportEmail}</p>
            </div>
            <div className="p-4 rounded-2xl bg-indigo-500/5 border border-indigo-500/15 space-y-1">
              <span className="text-2xl">💬</span>
              <p className="text-slate-400 block text-[10px]">{isRtl ? 'واتساب الدعم السريع' : 'WhatsApp Chat'}</p>
              <a 
      ...
      {activeTab === 'ticket' && (
        <form onSubmit={handleTicketSubmit} className="glass-card p-5 rounded-2xl border border-slate-200/40 dark:border-slate-800/40 space-y-4 text-xs">
          <h4 className="font-bold text-sm text-slate-800 dark:text-slate-200 border-b pb-2">{isRtl ? '📝 تقديم تذكرة دعم أو اقتراح ميزة' : '📝 Submit technical ticket or feature idea'}</h4>
          ...
          {ticketSuccess && (
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-center rounded-xl font-bold">
              {isRtl ? '✓ تم إرسال تذكرتك للدعم بنجاح وجاري فحصها.' : '✓ Ticket submitted successfully.'}
            </div>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-slate-400 block mb-1 font-bold">{isRtl ? 'نوع التذكرة' : 'Ticket Type'}</label>
              <select 
                value={ticketType}
              ...
              >
                <option value="bug">{isRtl ? 'الإبلاغ عن مشكلة فنية (Bug Report)' : 'Report technical issue'}</option>
                <option value="feature">{isRtl ? 'اقتراح ميزة إضافية (Feature Request)' : 'Suggest product feature'}</option>
              </select>
            </div>

            <div>
              <label className="text-slate-400 block mb-1 font-bold">{isRtl ? 'الموضوع / العنوان' : 'Subject'}</label>
              <input 
                type="text"
                required
                placeholder={isRtl ? 'عنوان مختصر للمشكلة أو المقترح' : 'Brief subject summary'}
                value={ticketSubject}
            ...
            <div className="md:col-span-2">
              <label className="text-slate-400 block mb-1 font-bold">{isRtl ? 'التفاصيل والشرح' : 'Explanation Details'}</label>
              <textarea 
                rows={4}
                required
                placeholder={isRtl ? 'اشرح بالتفصيل المشكلة أو الفكرة المطروحة...' : 'Detail the issue sequence or concept idea...'}
                value={ticketBody}
